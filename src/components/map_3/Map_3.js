...
import MarkerPlacement from "./MarkerPlacement_3";
...
function Map_3() {
  ...
  const pointToLayerAcademic = (feature, latlng) => {
    ...
    <div className="map">
      <MapContainer className="mapa_1" center={[52.2322222, 21.0]} zoom={10}>
        <LayersControl>
          ...
          <LayersControl.Overlay checked name="academic">
            {academic ? (
              <GeoJSON data={academic} pointToLayer={pointToLayerAcademic} />
            ) : (
              ""
            )}
          </LayersControl.Overlay>
